"""
Motors.co.uk scraper for CarTrack.

Navigates motors.co.uk search results using Playwright and extracts
...
MOTORS_BASE = "https://www.motors.co.uk/search/"
...
TITLE_SELECTOR = "h2.listing-title, h2[class*='title'], .listing-card__title, h2"
PRICE_SELECTOR = (
    "[data-testid='listing-price'], .listing-price, "
    ".price, [class*='price']"
)
LINK_SELECTOR = "a[href*='/used-cars/'], a[href*='/car-details/'], a.listing-card__link, a[href*='/cars/']"
IMAGE_SELECTOR = "img.listing-image, img[class*='thumbnail'], .listing-card__image img, img"
LOCATION_SELECTOR = (
    "[data-testid='listing-location'], .listing-location, "
    "[class*='location'], .seller-location"
)
NEXT_PAGE_SELECTOR = (
    "a[aria-label='Next'], a.pagination__next, "
    "button.pagination__next, [data-testid='pagination-next']"
)


def _build_url(params: dict, page: int = 1) -> str:
    """Construct a motors.co.uk search URL."""
    qs: dict[str, Any] = {
        "channel": "cars",
        "page": page,
    }
    ...
    if params.get("price_min"):
        qs["price_min"] = params["price_min"]
    if params.get("price_max"):
        qs["price_max"] = params["price_max"]
    if params.get("year_from"):
        qs["year_from"] = params["year_from"]
    if params.get("year_to"):
        qs["year_to"] = params["year_to"]
    ...
    if params.get("fuel_type"):
        qs["fuel"] = params["fuel_type"].lower()
    if params.get("postcode"):
        qs["location"] = params["postcode"]
    if params.get("radius"):
        qs["radius"] = params["radius"]
    else:
        qs["radius"] = 100

    return f"{MOTORS_BASE}?{urlencode(qs)}"


class MotorsScraper(BaseScraper):
    """Scrapes car listings from motors.co.uk."""

    source = "motors"

    async def scrape(self, search_params: dict) -> list[dict]:
        """
        Execute a search on motors.co.uk and return a list of listing dicts.
        ...
        async with self.get_browser() as (browser, context):
            ...
            self.logger.info("Motors: fetching page 1 — %s", first_url)
            ...
            for page_num in range(1, max_pages + 1):
                if page_num > 1:
                    ...
                    self.logger.info("Motors: fetching page %d — %s", page_num, url)
                ...
                try:
                    await page.wait_for_selector(
                        ", ".join(CARD_SELECTORS[:2]), timeout=10_000
                    )
                except Exception:
                    self.logger.warning("Motors: no listings selector found on page %d", page_num)

                page_listings = await self._extract_listings(page)
                self.logger.info(
                    "Motors page %d: extracted %d listings", page_num, len(page_listings)
                )
        ...
        self.logger.info("Motors scrape complete: %d total listings", len(results))
        return results
    ...
    async def _extract_listings(self, page) -> list[dict]:
        ...
        for sel in CARD_SELECTORS:
            ...
            if cards:
                self.logger.debug("Motors: using card selector '%s', found %d", sel, len(cards))
                break
        ...
        for card in cards:
            try:
                ...
                self.logger.debug("Motors: failed to parse card: %s", exc)
                continue

        return listings

    async def _parse_card(self, card) -> Optional[dict]:
        """Parse a single listing card into a dict."""
        ...
        if href.startswith("/"):
            href = f"https://www.motors.co.uk{href}"
        ...
        listing["sellerType"] = "private" if "private" in card_html.lower() else "dealer"
    ...
    async def _parse_specs(self, card, listing: dict) -> None:
        """Extract spec items from a Motors listing card."""
        listing["year"] = None
        ...
        if not spec_items:
            ...
            spec_items = [line.strip() for line in card_text.split("\n") if line.strip()]
    ...
    async def _has_next_page(self, page) -> bool:
        """Return True if a next-page navigation element is present and enabled."""
        ...
        if next_el:
            ...
            return visible and not disabled_attr and aria_disabled != "true"
        return False
